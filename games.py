...
def minmax_cutoff(game, state):
    ...
    def max_value(state, d):
        ...
        for action in game.actions(state):
            v = max(v, min_value(game.result(state, action), d - 1))
        return v
    ...
    def min_value(state, d):
        ...
        v = np.inf
        for action in game.actions(state):
            v = min(v, max_value(game.result(state, action), d - 1))
        return v
...
def alpha_beta_cutoff(game, state):
    ...
    for action in game.actions(state):
        value = min_value(game.result(state, action), alpha, beta, 0)
        if value > best_value:
            best_value = value
            best_action = action
...
def alpha_beta_player(game, state):
    ...
    if time.perf_counter() > 0.5 * game.timer:
...
class TicTacToe(Game):
    ...
    def eval1(self, state):
        ...
        # Count k-1 and k matches for both 'X' and 'O'
        x_k_minus_1 = sum(possiblekComplete(move, state.board, 'X', self.k-1) for move in state.moves)
        o_k_minus_1 = sum(possiblekComplete(move, state.board, 'O', self.k-1) for move in state.moves)
        x_k = sum(possiblekComplete(move, state.board, 'X', self.k) for move in state.moves)
        o_k = sum(possiblekComplete(move, state.board, 'O', self.k) for move in state.moves)


        evaluation = (x_k_minus_1 - o_k_minus_1) + 2 * (x_k - o_k)
